...
def cosine_similarity(x, y):
    ...
    return math.exp(cos_similarity)
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    if num_words == 0:
        return embedding  # Return zero vector if no words
    for word in words:
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        ##########################################
        openai_embedding = get_openai_embeddings(st.session_state.text_search, model_name=model_name)
        for idx, category in enumerate(categories):
            category_embedding = get_openai_embeddings(category, model_name=model_name)
            cosine_sim[idx] = cosine_similarity(openai_embedding, category_embedding)
        ...
        ##########################################
        transformer_embedding = get_sentence_transformer_embeddings(st.session_state.text_search, model_name=model_name)
        for idx, category in enumerate(categories):
            category_embedding = get_sentence_transformer_embeddings(category, model_name=model_name)
            cosine_sim[idx] = cosine_similarity(transformer_embedding, category_embedding)
...
if __name__ == "__main__":
    ...
    # )
    st.text_input(
        label="Categories", key="categories", value="Man woman cafe coffee"
    )
    ...
    text_search = st.text_input(
        ...
        key="text_search",
        value="Woman who owns cafe received a coffee order from man",
    )
    ...
    if st.session_state.text_search:
        ...
        st.write("")
        st.write(
            "Demo developed by [June Lim](https://www.linkedin.com/in/june97/ - Optional)"
        )
